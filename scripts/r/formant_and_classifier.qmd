...
```{r, echo=FALSE}
read.csv(here("data", "text_tidy.csv")) %>% 
  filter(f2 < 2000 & f1 < 750 & type == "fricative") %>% 
  select(fileID, sentence) %>% 
  separate(fileID, into = c("no", "version")) %>% 
  group_by(no, sentence) %>% 
  summarize(n = n()) %>% filter(n == 2) %>% select(-n) %>% 
  knitr::kable()
```
...
mid_formant_df_all = plot_data %>% select(type, f1, f2, f3)
...
### Midpoint classifier (all data)
```{r}
run_classifier(mid_formant_df_all, .7)
```
